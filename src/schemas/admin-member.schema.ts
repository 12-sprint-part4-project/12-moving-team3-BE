...
/** 관리자 회원 목록·상세 공통 필터 (page/pageSize 제외) */
const adminMemberFilterObjectSchema = z.object({
...
export const adminMemberListQuerySchema = listQuerySchema
  .extend(adminMemberFilterObjectSchema.shape)
  .extend({
...
/** GET /api/admin/members/:memberId query (목록과 동일, page/pageSize 제외, userType 필수) */
export const adminMemberDetailQuerySchema = adminMemberFilterObjectSchema
  .extend({
    // 회원/기사 화면이 분리되어 있어 neighbor 계산에 userType을 반드시 받는다.
    userType: z.enum(UserType),
  })
  .and(adminStatisticsFilterSchema);

export type AdminMemberDetailQuery = z.infer<
  typeof adminMemberDetailQuerySchema
>;
